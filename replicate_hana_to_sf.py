                ...
                if row['INCREMENTAL_FIELD'] == 'row_id':
                    ...
                    if ((snowflake_column_names_df_pandas.empty or (len(snowflake_column_names_df_pandas) == 0 and snowflake_column_names_df_pandas['COLUMN_NAME'].isnull().all()))) and row['LOAD_TYPE'] != 'FULL-REFRESH':
                        ...
                        if primary_key_list is None:
                            ...
                            logging.info("primary key does not exist so retrieving from athena table ")
                        if primary_key_list is not None:
                            ...
                            hana_table_partitioned = aws_functions.is_hana_table_partitioned(db_url, db_username, db_password, jdbc_driver_name, row['SOURCE1_SCHEMA_NAME'], row['SOURCE1_OBJECT_NAME'])
                            if hana_table_partitioned:
                                union_query = aws_functions.get_union_query_for_hana(db_url, db_username, db_password, jdbc_driver_name, row['SOURCE1_SCHEMA_NAME'], row['SOURCE1_OBJECT_NAME'])
                                hana_select_statement = f"({union_query}) AS t"
                                ...
                                PARTITIONED_INCREMENTAL_FIELD_MAX_VALUE = aws_functions.update_partitioned_max_row_id(sfOptions, row)
                                row['PARTITIONED_INCREMENTAL_FIELD_MAX_VALUE'] = PARTITIONED_INCREMENTAL_FIELD_MAX_VALUE
                            else:
                                no_of_records_processed, snowflake_start_time, snowflake_end_time, hana_start_time, hana_end_time, recon_start_time, recon_end_time, delta_record_count, delta_record_samples = aws_functions.hana_to_snowflake_insert(hana_select_statement, db_url, db_username, db_password, jdbc_driver_name, predicates_list, row, sfOptions)
                        ...
                        if primary_key_list is not None:
                            ...
                            hana_table_partitioned = aws_functions.is_hana_table_partitioned(db_url, db_username, db_password, jdbc_driver_name, row['SOURCE1_SCHEMA_NAME'], row['SOURCE1_OBJECT_NAME'])
                            row['HANA_TABLE_PARTITIONED'] = hana_table_partitioned

                            if hana_table_partitioned:
                                union_query = aws_functions.get_union_query_for_hana(db_url, db_username, db_password, jdbc_driver_name, row['SOURCE1_SCHEMA_NAME'], row['SOURCE1_OBJECT_NAME'])
                                hana_union_select_query_pt= f"({union_query}) AS t"
                                hana_select_statement = aws_functions.map_query_with_partition_data(hana_union_select_query_pt, row['PARTITIONED_INCREMENTAL_FIELD_MAX_VALUE'])
                                logging.info(f"HANA Query with partitioned table is {hana_select_statement}")
                                predicates_list = ['1=1']
